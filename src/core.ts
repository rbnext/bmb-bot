...
export const purchaseGoodsById = async ({
  ...
  for (const filteredGood of sellOrders.data.items) {
    const profit = Number(sellReferencePrice) - Number(filteredGood.price)
    ...
    await logger({ message: `Purchase "${marketHashName}". Profit: ~${profit.toFixed(2)}$` })
